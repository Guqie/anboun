...
def get_a_interpret_source_text_prompt(json_dumps_result):
    return f"""# 解读信源文本
...
# 为了向后兼容，创建一个包装函数
def a_interpret_source_text(**kwargs):
    if 'json_dumps_result' in kwargs:
        return get_a_interpret_source_text_prompt(kwargs['json_dumps_result'])
    else:
        # 如果没有传递json_dumps_result参数，使用默认的json格式
        json_dumps_result = json.dumps(json_format, indent=4, ensure_ascii=False)
        return get_a_interpret_source_text_prompt(json_dumps_result)
