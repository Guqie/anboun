...
json_format = {
  "1": {
    ...
    "有无重复": "这是第1句，所以没有重复",
    "逻辑连贯性": "检查本句与其前一句的逻辑关系是否清晰、自然。是否存在逻辑跳跃、指代不明或连接生硬的问题？输出思考过程，再回答“逻辑连贯”，或指出具体的修改建议（如“建议在句首增加连接词”或“建议将此句拆分”）。"
...
- 第一，请严格基于source_text，按照序号依次检查简报内容的每一句有没有捏造事实信息，有没有遗漏事实信息，有没有与前面各句重复的信息，**以及与前一句的逻辑连贯性**，以JSON格式呈现对每一句的检查结果，并包裹在XML标签<feedback_on_brief_sentences>中输出。
...
- 如果发现句子之间的逻辑连接生硬或不清晰，应明确指出并提出修改建议。
- **核心原则**你的首要职责是确保**事实的准确性（无捏造）**和**信息的无冗余（无重复）**。
- **[关于“遗漏”的动态标准]**: 在检查“遗漏 (omission)”时，你**必须 (MUST)** 遵循以下动态标准：
    - **理想篇幅目标**: 600字符。
    - **IF** 整篇简报明显偏短（例如，你感觉总共只有3-4句话，远未达到理想篇幅）
    - **THEN** 你应该采取**更严格的“遗漏”检查标准**。对于那些能够提供重要背景、增强论证的细节，你**应当 (SHOULD)** 标记为“有遗漏”并建议补充。
    - **ELSE IF** 整篇简报明显偏长（例如，你感觉内容非常密集，可能已接近或超过篇幅上限）
    - **THEN** 你应该采取**更宽松的“遗漏”检查标准**。只有当省略的细节**严重扭曲了原文核心意思**时，才标记为“有遗漏”。对于其他为了简洁而进行的合理省略，应标记为“**为保持简洁，可接受的省略**”。
    - **ELSE** (篇幅适中)
    - **THEN** 你可以根据专业判断进行裁定。
...
- 为了避免遗漏任何细节信息，最好的方法是尽可能保持原文的详细程度。
